...
m1 <- vlbw %>%
  ...
  tm_fill('rate', 
          ...
          title = '% VLBW') +
  tm_borders() +
  tm_layout(title = 'VLBW in Georgia, 2018-2019',
...
# Now add a variable to the dataset representing expected and SMR
vlbw <- vlbw %>%
...
A function to estimate the continuous *p-value* associated with the SMR is the `probmap` function from the package `spdep`.  This function calculates the probabilities (under the Poisson probability model) of observing an event count *more extreme* than what was actually observed, given the expected count (e.g. we might expect every county had the overall risk). The test is a one-tailed test, focused by default on SMR that are above 1 (to test for extremes below 1, set the argument `alternative = 'greater'`). 

`probmap` expects several arguments including a vector of the count of cases, a vector of the population at risk, and optionally a `row.names` vector to help align observations. Because your job is to identify counties with SMR in excess of expected (e.g. >>1), the default alternative hypothesis is appropriate. 
...
```{r message=F, warning=F}
pv <- vlbw %>%
  mutate(pmap.pv = ifelse(SMR > 1 & pmap > 0.975, 1, 0)) %>%
  group_by(pmap.pv) %>%
  summarise() %>%
  filter(pmap.pv == 1)
...
# names(ee)  # look at the object returned
...
While not necessary for disease mapping, you might be interested in how different the *Bayesian* and *frequentist* approach are. This plot shows that for this dataset, they are largely consistent, but not identical to one another. The differences reflect the smoothing or stabilization due to the EB methods.
...
pv2 <- vlbw %>%
  mutate(eb.pv = ifelse(ebSMR > 1 & eb2_prob > 0.975, 1, 0)) %>%
  group_by(eb.pv) %>%
  summarise() %>%
  filter(eb.pv == 1)
...
m4 <- tm_shape(vlbw) +
  tm_fill('ebSMR',
          ...
          title = 'Std. Morbidity Ratio') + 
  tm_borders() +
  tm_layout(main.title = 'Empirical Bayes smoothed SMR of VLBW',
            inner.margins = c(0.1, 0.02,0.05,0.2),
            legend.format = list(digits = 2)) +
  # Add dark borders for significant
  tm_shape(pv2) +
