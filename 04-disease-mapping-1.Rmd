...
print_learn_objective('Disease Mapping I')
```
...
age <- st_read('../SpatialEpi-2021/DATA/VLBW-GA/ga-vlbw-age.gpkg')
```
...
m1 <- vlbw %>%
  ...
  tm_fill('rate', 
          ...
          title = '% VLBW') +
  tm_borders() +
  tm_layout(main.title = 'All data',
            inner.margins = c(0.1, 0.02, 0.1, 0.05),
            legend.format = list(digits = 1)) +
  tm_credits("Very low birth weight (<1500 grams), 2018-2019, \nNo suppression",
...
m2 <- vlbw %>%
  ...
  tm_fill('rate', 
          ...
          title = '% VLBW') +
  tm_borders() +
  tm_layout(main.title = 'Suppress sparse counts',
            inner.margins = c(0.1, 0.02, 0.1, 0.05),
            legend.format = list(digits = 1)) +
  tm_credits("Very low birth weight (<1500 grams), 2018-2019, \nsuppressing counties with < 10 events",
             position = c("RIGHT","BOTTOM"))
...
age %>%
  arrange(GEOID) %>%
  dplyr::select(GEOID, NAME, AGECAT, VLBW, TOT) %>%
  st_drop_geometry() %>%
  head()
```
